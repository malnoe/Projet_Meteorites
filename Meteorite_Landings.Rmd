...
# Télécharger les données de pays
world <- ne_countries(scale = "medium", returnclass = "sf")

# Convertir meteorite en objet spatial
meteorite <- st_as_sf(meteorite, coords = c("reclong", "reclat"), crs = 4326, remove = FALSE)

# Ajouter la colonne pays (ici en anglais, depuis la colonne 'name')
meteorite <- st_join(meteorite, world["name"])

names(meteorite)[names(meteorite) == "name.y"] <- "pays"
meteorite$pays_fr <- countrycode(
  meteorite$pays,
  origin = "country.name",
  destination = "cldr.name.fr"
)
...
```{r}
meteorite <- na.omit(meteorite)
meteorite <- meteorite_clean[meteorite_clean$year != 860,]
```
...
meteorite <- na.omit(meteorite)
meteorite <- meteorite[meteorite != 860,]
world <- ne_countries(scale = "medium", returnclass = "sf")
meteorite <- st_as_sf(meteorite, coords = c("reclong", "reclat"), crs = 4326, remove = FALSE)
meteorite <- st_join(meteorite, world["name"])
names(meteorite)[names(meteorite) == "name"] <- "pays"
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    selectInput("pays_filter", "Pays :", 
                choices = sort(unique(na.omit(meteorite_clean$pays_fr))),
