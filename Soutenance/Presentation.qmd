...
- Intensité constante : λ(x,y) = λ₀  
- Points indépendants entre eux  
- Répartition uniforme dans l'espace  
- Nombre de points dans une zone A suit :  
  N(A) ~ Poisson(λ₀ × aire(A))
...
- Intensité variable : λ(x,y) dépend de la position  
- Points indépendants entre eux  
- Densité non-uniforme dans l'espace  
- Nombre de points dans une zone A suit :  
  N(A) ~ Poisson(∫∫ₐ λ(x,y) dxdy)
